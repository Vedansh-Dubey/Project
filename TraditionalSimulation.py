...
speeds = {'car':1.5, 'bus':1.5, 'truck':1.5, 'rickshaw':1.5, 'bike':1.5}  # average speeds of vehicles
...
def generateVehicles():
    # Vehicles per second
    direction_frequencies = [0.75, 1.5, 1, 1.25]
    ...
    while True:
        ...
        direction_number = random.choices(range(4), weights=direction_frequencies)[0]

        time.sleep(1 / direction_frequencies[direction_number])

        vehicle_type = random.randint(0, 3)  # Excluding the "will_turn" type
        Vehicle(
            lane_number,
...
class Main:
    ...
    background = pygame.image.load('images/katolNaka.png')
